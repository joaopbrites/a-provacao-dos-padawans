...
test("selection questions: estrutura basica e distribuicao por nivel", () => {
  const ids = new Set();
...
test("selection questions: integracao de snapshotField, template e fromArray", () => {
  const snapshots = generateAllSnapshots(
    (arr) => runSelectionSort(arr, selectionSortMapping.lineMap, 808),
    [7, 2, 5, 1]
  );
  ...
  for (const question of selectionSortQuestions) {
    ...
    for (const idx of indexes) {
      const resolved = resolveTargetSnapshot(idx, question.offsetRange, snapshots, mulberry32(500 + idx));
      if (!resolved) {
        continue;
      }
...
test("selection questions: validacao estatica do sistema", () => {
  const errors = validateQuestionDefinitions(selectionSortQuestions, "selection-sort");
